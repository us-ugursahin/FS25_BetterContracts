...
function BCsetting.new(data, customMt)
	...
	if data.values ~=nil and next(data.values) ~=nil then
		table.move(data.values,1,#data.values,1, self.values)
		table.move(data.texts,1,#data.texts, 1, self.texts)
	elseif data.min ~= nil and data.max ~=nil then
		...
		table.move(self.data.values,1,#self.data.values,1, self.values)
		if self.data.texts ~= nil then
			table.move(self.data.texts,1,#self.data.texts, 1, self.texts)
		end
	end
...
function BCsetting:setValue(value)
	...
	if not self.values then
		Logging.warning("[BetterContracts] %s:setValue() called with nil value range",self.name)
		return false
	end
